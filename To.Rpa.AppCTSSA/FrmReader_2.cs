...
namespace To.Rpa.AppCTSSA
{
    public partial class FrmReader_2 : Form
    {
        ...
        private void Crop()
        {
            ...
            double xfactor = 4677.00 / SrcPicBox.Size.Width;
            double yfactor = 3307.00 / SrcPicBox.Size.Height;

            rectCropArea.X = Convert.ToInt32(Math.Round(rectCropArea.X * xfactor));
            rectCropArea.Y = Convert.ToInt32(Math.Round(rectCropArea.Y * yfactor));
            rectCropArea.Height = Convert.ToInt32(Math.Round(rectCropArea.Height * yfactor));
            rectCropArea.Width = Convert.ToInt32(Math.Round(rectCropArea.Width * xfactor));
